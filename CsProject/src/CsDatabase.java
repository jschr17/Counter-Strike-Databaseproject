...
public class CsDatabase {
    ...
    private void assignmentA() {
        ...
        try {
            ...
            System.out.printf("|%1$-35s|%2$-35s|%n", rs.getMetaData().getColumnName(1), rs.getMetaData().getColumnName(2));
            while (rs.next()) {

                System.out.printf("|%1$-35s|%2$-35s|%n", rs.getString(1), rs.getString(2));
            }
            rs.close();
            st.close();
